...
export const keys: Key[] = [
    ...
    new ExtraTallKey(new AlternateFontKey(new PlainSymbolKey('⎋', Row.Thumbs, 'escape'))),
    ...
    new AlternateFontKey(new PlainSymbolKey('⎀', Row.Thumbs, 'insert')),
    ...
    new AlternateFontKey(new PlainSymbolKey('⌃', Row.Mods, 'control')),

    // Two super keys (opposite thumbs)
    new PlainSymbolKey('❖', Row.Mods, 'super'),
    new AlternateFontKey(new PlainSymbolKey('⎄', Row.Mods, 'compose').size(5)), // RGUI

    // Two alt keys (opposite thumbs)
    new AlternateFontKey(new PlainSymbolKey("⎇", Row.Mods, 'alt')),
    new PlainSymbolKey("⇮", Row.Mods, 'altgr'),
];

keys.sort((a, b) => rowToNumber(a.row) - rowToNumber(b.row) || a.id.localeCompare(b.id));
